...
class CreateReportSchedulerTaskParser:
    ...
    def parse_utc_time(self, start_time):
        try:
            time_zone = config.TIME_ZONE.get('asia_taipei')
            naive_result_time = self.get_compare_time(start_time)
    ...
    def parse_local_time(self, start_time):
        try:
            return self.get_compare_time(start_time)
        except Exception as e:
            traceback.print_exc()
            error_log(traceback.format_exc())
            raise e

...
class DirNodeDiffObject:
    ...
    def parse_left_only_node(self):
        ...
        for name in self.left_only:
            self.parse_environment_only_node(name, self.left_ftp_connect_obj, self.left_entry_map, self.left_node,
                                             self.right_node)
    ...
    def parse_right_only_node(self):
        ...
        for name in self.right_only:
            self.parse_environment_only_node(name, self.right_ftp_connect_obj, self.right_entry_map, self.right_node,
                                             self.left_node)

    def parse_environment_only_node(self, name, ftp_connect_obj, entry_map, parent_node_add, parent_node_remove):
        node_add = self.parse_node(name, entry_map, parent_node_add)
        node_add.set_diff_result(KEY_D_RESULT_ADD)

        node_remove = self.parse_node(name, entry_map, parent_node_remove)
    ...
    def parse_union_node(self):
        ...
        for name in self.union:
            left_node = self.parse_node(name, self.left_entry_map, self.left_node)
            left_node.set_diff_result(KEY_D_RESULT_SAME)

            right_node = self.parse_node(name, self.right_entry_map, self.right_node)
...
class DirNodeDiffFilterObject(DirNodeDiffObject):
    ...
    def diff(self):
        try:
            left_entry_list = self.left_ftp_connect_obj.get_path_list_dir(self.left_node.path)
            right_entry_list = self.right_ftp_connect_obj.get_path_list_dir(self.right_node.path)
            self.parse_filter_node_object(left_entry_list, self.left_entry_map, self.left_name_set, self.left_filters)
    ...
    def parse_node(self, name, entry_map, parent_node):
        return super().parse_node(name, entry_map, parent_node)
...
class DirNodeDiffLastVersionObject(DirNodeDiffFilterObject):
    ...
    def diff_last_version_node(self):
        left_node = self.parse_node(self.left_filters[0], self.left_entry_map, self.left_node)
        left_node.set_diff_result(KEY_D_RESULT_SAME)

        right_node = self.parse_node(self.right_filters[0], self.right_entry_map, self.right_node)
        right_node.set_diff_result(KEY_D_RESULT_SAME)
